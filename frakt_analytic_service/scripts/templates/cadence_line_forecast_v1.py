# service_python/scripts/templates/cadence_line_forecast_v1.py
"""
Frakt SVG Template: cadence_line_forecast_v1

A general-purpose line chart with an optional dashed forecast extension.
Designed for the Cadence <-> Frakt integration:
...
Sandbox constraints (RestrictedPython, see worker/generator.py):
...
This template therefore consumes lists destructively via `.pop(0)` inside
...
REQUIRED_PARAMS = {
    "points": "list[[x, y]] - history line coordinates, auto pixel-mapped by /v1/generate and /v1/generate-predictive",
    "forecast_x": "list[float] - forecast pixel x-coordinates, only present on /v1/generate-predictive responses",
    "forecast_y": "list[float] - forecast pixel y-coordinates, only present on /v1/generate-predictive responses",
    "stroke_color": "str - hex color for both the history and forecast line, e.g. '#2ecc71'",
    "title (metadata)": "str - optional chart title stamped in the top-left corner",
}
